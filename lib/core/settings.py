...
# Enhanced sqlmap with auto-installer and shell integration
# Original: sqlmap developers (https://sqlmap.org)
# ==============================================================================

# sqlmap version (<major>.<minor>.<month>.<monthly commit>)
VERSION = "1.10.6.12"
TYPE = "dev" if VERSION.count('.') > 2 and VERSION.split('.')[-1] != '0' else "stable"
TYPE_COLORS = {"dev": 31, "stable": 90, "pip": 34}  # Recoded: dev type uses red (31) for Xbibz branding
VERSION_STRING = "sqlmap/%s#%s" % ('.'.join(VERSION.split('.')[:-1]) if VERSION.count('.') > 2 and VERSION.split('.')[-1] == '0' else VERSION, TYPE)
DESCRIPTION = "automatic SQL injection and database takeover tool [Recoded By Xbibz Official]"
SITE = "https://sqlmap.org"
RECODED_BY = "Xbibz Official"
RECODED_VERSION = "3.0"
...
# colorful banner - Recoded By Xbibz Official
BANNER = """\033[01;33m\
        ___
       __H__
...
\033[01;31m    [Recoded By Xbibz Official v%s]\033[0m
""" % (TYPE_COLORS.get(TYPE, 31), VERSION_STRING.split('/')[-1], SITE, RECODED_VERSION)
