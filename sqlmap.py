...
Enhanced with auto-installer, shell integration (.bashrc/.zshrc),
and curl-based installation support.
"""
...
def main():
    ...
    try:
        ...
        # Recoded By Xbibz Official: --auto mode initialization
        if conf.get("auto"):
            from lib.controller.auto import autoInit
            autoInit()

        # Recoded By Xbibz Official: --spider mode initialization
        if conf.get("spider"):
            from lib.controller.auto import autoSpiderInit
            autoSpiderInit()
